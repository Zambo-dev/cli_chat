...
int server_accept(server_t *server)
{
	...
	while((idx = list_get_free(server->list)) == -1);
...
int server_read(server_t *server)
{
	...
	if(select(maxfd+1, &sigfd, NULL, NULL, &tv) > 0)
	{
		for(int i=0; i<CONNLIMIT; ++i)
		{
			if(server->list[i] != NULL && FD_ISSET(server->list[i]->fd, &sigfd))
			{
...
int server_write(server_t *server, char *buffer, size_t bytes)
{
	...
	if(select(maxfd+1, NULL, &sigfd, NULL, &tv) > 0)
	{
		for(int i=0; i<CONNLIMIT; ++i)
		{
			if(server->list[i] != NULL && FD_ISSET(server->list[i]->fd, &sigfd))
